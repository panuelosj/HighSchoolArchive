...
void black_black_check(void)
{
	if(readSensL >= thresh && readSensR >= thresh)
	{
		...
		//------------------CASES---------------------------------------
			...
			else if(blackBlackCounter == 2 || blackBlackCounter == 4) 
			{
				move_stop();
				servo_on();
				_delay(sec);
...
void PID_controller(void)
{
	...
	derivativeError = error - pastError;			//Derivative
	D_turn = Kd * derivativeError;				
	pastError = error;
	
	integralError += error;							//Integral
	I_turn = Ki * integralError;	
	
	P_turn = Kp * error;							//Proportional
...
void move_forwards_aLittle(void)		//really? just move forwards for a little bit
{
	leftF = 1;
	leftB = 0;
	rightF = 1;
...
void move_180(void)
{
	...
	rightF = 0;
	rightB = 1;
	_delay(sec*2);							//100000	//sec
	leftF = 1;							//turn a bit to prevent checking the perpendicular
	leftB = 0;
	rightF = 0;
	rightB = 1;
	_delay(sec*3);						//1500000	//sec*3/2
	ADC_convert();
	while(readSensR < 0b10110000)		//continue turning until black is seen
	{
		ADC_convert();
		_delay(50);
	}
