/* -------------------------------------------------------------------------------------------
	...
	UPDATE: added Proportional Calculations
			trying to use only one light sensor
		Hardware setup:
...
#define thresh 0b01111111			//analog treshold between black and white
#define sec 1000000
...
void main(void)
{
	...
	while(loopExit == 1)
	{
		...
		else								//resets cycle back to 0
		{
			...
			ADCON0 = 0b00000000;				//turns off ADC to conserve power
			...
			//-------------SINGLE LIGHT SENSOR CONFIG---
			
			
			
			//-------------DUAL LIGHT SENSOR CONFIG-----
			
		//	readSensR = readSensR + 0b00010100;			//offset to equalize difference in sensor readings
		//	readSensR = readSensR * 9 / 10;
		//	readSensR = readSensR + 25;
										
			if(readSensL > readSensR)					//Left darker than Right
			{											//Turn Left
				deltaSens = readSensL - readSensR;
				if(deltaSens > 100)
				{
					deltaSens = 100;
				}
			//	deltaSens = 2 * deltaSens / 3;
				dutyMotorL = 100 - deltaSens;
				dutyMotorR = 100;
				sensR = 0;
				sensL = 1;
			}
			
			if(readSensR > readSensL)					//Right darker than Left
			{											//Turn Right
				deltaSens = readSensR - readSensL;
