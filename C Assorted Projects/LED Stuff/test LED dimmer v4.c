...
#define pressed 1					// -----------------WARNING - CHECK BASED ON HARDWARE
#define unpressed 0					// -----------------WARNING - CHECK BASED ON HARDWARE
...
int pWidthBlue = 30;	//pulse width of blue
int pWidthGreen = 100;	//pulse width of green
...
void main(void)
{
	...
	LEDred = 1;
	for(i=0; i<8; i++)			//program start animation
	{
		PORTC = ledArray[i];
		_delay(sec/35);
	}
